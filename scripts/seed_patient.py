...
def _upsert_eleanor(session: Session) -> None:
    ...
    print(f"Seeded patient #{profile.id}: {profile.name} ({profile.age})")


def main() -> None:
    """Populate Patient + EmergencyContact + Medication for demo scenarios."""
    init_db()
    with Session(engine) as session:
        _upsert_eleanor(session)
    print("Seed complete.")
